...
namespace m5avatar
{
  ...
  void Eye::draw(M5Canvas *spi, BoundingRect rect, DrawContext *ctx)
  {
    ...
    if (openRatio > 0)
    {
      ...
      if (exp == Expression::Happy || exp == Expression::Cry)
      {
        int x0, y0, w, h;
        x0 = x + offsetX - r;
        y0 = y + offsetY - r;
      ...
      if (exp == Expression::Gangimari)
      {
        // ガンギマリの目を描画（大きく見開いた目）
        spi->fillCircle(x + offsetX, y + offsetY, r * 1.2, primaryColor);
        spi->fillCircle(x + offsetX, y + offsetY, r * 0.6, backgroundColor);
        // 瞳孔を小さく
        spi->fillCircle(x + offsetX, y + offsetY, r * 0.2, primaryColor);
      }
    }
    else
    {
